...
struct write_arg { __u64 buf; __u32 len; };
struct ex_arg { __u64 buf; __u64 num; __u64 written; };
...
static __always_inline int push_event(u8 type, __u64 buf, u32 len) {
    if (len == 0) return 0;
...
int uprobe_ssl_write_ret(struct pt_regs *ctx) {
    ...
    if (!arg) return 0;
    push_event(SSL_WRITE, arg->buf, arg->len);
...
int uprobe_ssl_read_entry(struct pt_regs *ctx) {
    if (!is_agent()) return 0;
    u64 id = bpf_get_current_pid_tgid();
    struct write_arg arg = { .buf = (__u64)PT_REGS_PARM2(ctx), .len = 0 };
    bpf_map_update_elem(&write_args, &id, &arg, BPF_ANY);
    return 0;
}
...
int uprobe_ssl_read_ret(struct pt_regs *ctx) {
    ...
    push_event(SSL_READ, arg->buf, (u32)ret);
    bpf_map_delete_elem(&write_args, &id);
    return 0;
}
...
int uprobe_ssl_write_ex_ret(struct pt_regs *ctx) {
    ...
    if (written > 0) push_event(SSL_WRITE, arg->buf, (u32)written);
...
int uprobe_ssl_read_ex_ret(struct pt_regs *ctx) {
    ...
    if (readbytes > 0) push_event(SSL_READ, arg->buf, (u32)readbytes);
    bpf_map_delete_elem(&ex_args, &id);
    return 0;
}
